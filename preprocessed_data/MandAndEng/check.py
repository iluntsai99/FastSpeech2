...
for i, line in enumerate(tqdm(lines)):
    wav_name, speaker, text, _, lang = line.split('|')
    lang = lang.strip()
    if lang == "zh":
        wav_path = os.path.join(in_dir, "AISHELL3", speaker, wav_name+".wav")
    else:
        wav_path = os.path.join(in_dir, "LibriTTS", speaker, wav_name+".wav")
